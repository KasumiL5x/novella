...
class ContextLinkableView: LinkableView {
	...
	override func draw(_ dirtyRect: NSRect) {
		...
		if let context = NSGraphicsContext.current?.cgContext {
			...
			let bgColors = [Trashed ? Settings.graph.nodes.contextStartColor.withSaturation(Settings.graph.trashedSaturation).cgColor : Settings.graph.nodes.contextStartColor.cgColor,
