...
class NVVariable: NVIdentifiable {
	...
	var Name: String {
		didSet {
			...
			_story.Delegates.forEach{$0.nvVariableNameDidChange(story: _story, variable: self)}
		}
	}
	var Constant: Bool {
		didSet {
			NVLog.log("Variable (\(UUID.uuidString)) Constant changed (\(oldValue) -> \(Constant)).", level: .info)
			_story.Delegates.forEach{$0.nvVariableConstantDidChange(story: _story, variable: self)}
		}
	}
	...
	func set(value: NVValue) {
		...
		_story.Delegates.forEach{$0.nvVariableValueDidChange(story: _story, variable: self)}
	}
	...
	func set(initialValue: NVValue) {
		...
		_story.Delegates.forEach{$0.nvVariableInitialValueDidChange(story: _story, variable: self)}
	}
