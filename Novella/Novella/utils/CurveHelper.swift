...
//  novella
//
//  Created by Daniel Green on 11/12/2018.
...
class CurveHelper {
	static func catmullRom(points: [NSPoint], alpha: CGFloat, closed: Bool) -> NSBezierPath {
	...
	static func line(start: NSPoint, end: NSPoint) -> NSBezierPath {
		let path = NSBezierPath()
		path.move(to: start)
		path.line(to: end)
		return path
	...
	static func square(start: NSPoint, end: NSPoint) -> NSBezierPath {
		let path = NSBezierPath()
		let spacing: CGFloat = 20.0
